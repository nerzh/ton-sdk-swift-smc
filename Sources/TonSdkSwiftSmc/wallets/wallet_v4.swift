...
public class WalletV4 {
    ...
    public var subWalletId: UInt32

    public init(pubkey: Data, wc: Int = 0, subWalletId: UInt32 = WalletV4.SUB_WALLET_ID) throws {
        self.pubkey = pubkey
        self.subWalletId = subWalletId
        guard let code = try Serializer.deserialize(data: WalletV4.WALLET_V4_CODE.hexToBytes()).first else {
    ...
    public func buildTransfer(transfers: [WalletV4Transfer], seqno: UInt32, privateKey: Data, isInit: Bool = false, timeout: UInt = 60) throws -> Message {
        ...
        let initValue = isInit ? self.initValue : nil

        return try Message(options: MessageOptions(info: info, initOptions: initValue, body: messageBody.cell()))
    }
    ...
    private static func buildStateInit(code: Cell, pubkey: Data, subWalletId: UInt32) throws -> StateInit {
        ...
        return try StateInit(stateInitOptions: try StateInitOptions(code: code, data: data.cell()))
    }
}
